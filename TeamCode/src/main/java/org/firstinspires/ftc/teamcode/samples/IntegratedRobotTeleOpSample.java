...
@TeleOp(name = "IntegratedRobotTeleOpSample", group = "TeleOp")
public class IntegratedRobotTeleOpSample extends LinearOpMode {
    ...
    private void driveLinear(double axial, double lateral, double yaw) {
        ...
        double leftDriveFrontPower  = axial + lateral + yaw;
        double rightDriveFrontPower = axial - lateral - yaw;
        double leftDriveRearPower   = axial - lateral + yaw;
        double rightDriveRearPower  = axial + lateral - yaw;

        // Normalize the values so no wheel power exceeds 100%
        // This ensures that the robot maintains the desired motion.
        max = Math.max(Math.abs(leftDriveFrontPower), Math.abs(rightDriveFrontPower));
        max = Math.max(max, Math.abs(leftDriveRearPower));
        max = Math.max(max, Math.abs(rightDriveRearPower));

        if (max > 1.0) {
            leftDriveFrontPower  /= max;
            rightDriveFrontPower /= max;
            leftDriveRearPower   /= max;
            rightDriveRearPower  /= max;
        }
        ...
        if (gamepad1.x || gamepad1.a || gamepad1.y || gamepad1.b) {
            leftDriveFrontPower = gamepad1.x ? 1.0 : 0.0;  // X gamepad
            leftDriveRearPower = gamepad1.a ? 1.0 : 0.0;  // A gamepad
            rightDriveFrontPower = gamepad1.y ? 1.0 : 0.0;  // Y gamepad
            rightDriveRearPower = gamepad1.b ? 1.0 : 0.0;  // B gamepad
        }
        // Send calculated power to wheels
        leftDriveFront.setPower(leftDriveFrontPower);
        rightDriveFront.setPower(rightDriveFrontPower);
        leftDriveRear.setPower(leftDriveRearPower);
        rightDriveRear.setPower(rightDriveRearPower);

        // Show the elapsed game time and wheel power.
        telemetry.addData("Status", "Run Time: " + runtime.toString());
        telemetry.addData("Front left/Right", "%4.2f, %4.2f", leftDriveFrontPower, rightDriveFrontPower);
        telemetry.addData("Back  left/Right", "%4.2f, %4.2f", leftDriveRearPower, rightDriveRearPower);
    }
}
